...
+++
title = "S式でワンライナーを書くための行指向ミニ言語『cho』"
date = 2026-09-20
+++


[`cho`](https://github.com/k0f1sh/cho) というコマンドラインツールを作りました。
choはawkを参考にした小さなテキスト処理ツールです。通常は標準入力を1行ずつ読み込み、行の絞り込みや列の取り出し、加工をS式で記述できます。
...
### 1. awkのように、行の読み込みと列への分割を任せられる

「Lispでテキスト処理をする」だけであれば、SchemeやBabashkaを使うほうがよいと思います。

しかし、行を読み込むだけでなく、列に分割して値を取り出すところまで書くと、ワンライナーは長くなりがちです。
choではawkのように入力をレコードとフィールドに分割し、`$1`、`$2` で参照できるようにするところまでをやってくれます。
...
この例では、2列目を数値として比較し、応答時間が20ミリ秒を超えた行のサービス名とパスを出力します。

デフォルトでは1行が1レコードとなり、連続する空白文字がフィールドの区切りになります。区切りを変えたい場合は、`-F` で正規表現を指定できます。

CSVやTSVにも対応しています。CSVモード（`--csv`）では、引用符で囲まれたフィールド内のカンマや改行も扱えます。次の例では、`Tokyo, Japan` がひとつのフィールドになります。
...
awkの行や列を手軽に扱えるところが好きで、その処理を自分になじみのあるS式で書けたら便利だと考えたのが、choを作り始めたきっかけです。
...
awkでいう `awk '$2 > 20 {print $1, $3}'` に相当します。

choは、スクリプトを書くほどではないテキスト処理を、ワンライナーで済ませたいときに使うことを想定しています。そのため、配列、ループ、ユーザー定義関数、代入、`BEGIN` / `END` ブロックは用意していません。こうした機能が必要になる処理には、スクリプト言語を使うほうがよいと思っています。

出現頻度のカウントやソートなど、複数のレコードをまとめて扱う処理は従来通り、`sort` や `uniq` などのコマンドと組み合わせて行います。
...
ワンライナーでテキスト処理をしていると、文字列や数値だけでなく、日時やIPアドレス、URL、単位付きのファイルサイズなどを扱いたい場面が頻繁に出てきます。

日時の比較ではタイムゾーンを考慮する必要がありますし、IPアドレスがCIDRで指定したネットワークに含まれるかは、文字列の比較だけでは判定できません。ライブラリを使えば処理できますが、ワンライナーでも手軽に書きたいところでした。
...
各フィールドは文字列として読み込まれ、関数が求める型に応じて自動的に変換されます。たとえば `cidr/contains?` に渡す `$1` はIPアドレスとして、`bs/>` に渡す `$2` はバイトサイズとして解釈されます。

型変換に失敗した場合は、通常はその時点でエラー終了します。不正な値を黙って読み飛ばし、ログの形式が崩れていることを見落とすのを防ぐためです。
