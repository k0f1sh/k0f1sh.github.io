...
- 日付や日時を比較し、日数の加算や日時の差分を計算する（`d/>=`、`d/add`、`dt/diff`）
- `MB` や `MiB` などの単位が付いたサイズを比較し、バイト数に変換する（`bs/>=`、`bs/to-b`）
- IPv4・IPv6アドレスを正規化し、CIDRで指定したネットワークに含まれるかを判定する（`ip`、`cidr/contains?`）
- URLからホスト名やパスを取り出す（`url/host`、`url/path`）

このほか、SemVerの比較やUUID・ULIDの生成にも対応しています。

たとえば、1列目がIPアドレス、5列目がURLのログなら、次のようにネットワークで絞り込み、IPアドレスとURLのパスを出力できます。
